...
refresh_button_re = r"button:refresh:(?P<gamemode>ranked|unranked|warmup|casual):(?P<platform>xbl|psn|uplay):(?:(?P<identifier>(?!.*\s{2})([a-zA-Z][a-zA-Z0-9 ]{0,12})|(?=.*[a-zA-Z])[a-zA-Z][a-zA-Z0-9_-]{2,15}|(?!.*\b(?:ubi|ubi$))(?!^\d)[a-zA-Z][a-zA-Z0-9_.-]{2,14})|(?P<id>[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}))"

class RefreshButton(discord.ui.DynamicItem[discord.ui.Button], template=refresh_button_re):
    bot: Optional[BotU] = None
    def __init__(self, gamemode: str, platform: Platform, username_or_id: str, /):
        self.gamemode = gamemode
        self.platform = platform
        if is_valid_id(username_or_id):
            self.id = username_or_id
            self.username_or_id = None
        else:
        ...
        super().__init__(
            discord.ui.Button(
                ...
                custom_id=f"button:refresh:{self.gamemode}:{platform.route}:{username_or_id}",
                emoji=f"{emojidict.get('refresh')}"
            ),
        )
    ...
    async def from_custom_id(cls, interaction: discord.Interaction, item: discord.ui.Item, match: re.Match[str], /):
        ...
        platform = Platform.from_str(str(match['platform']))
        if match['id']:
            instance = cls(gamemode, platform, match['id'])
        else:
            instance = cls(gamemode, platform, match['identifier'])

        if isinstance(interaction.client, BotU):
            instance.bot = interaction.client
        return instance
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        if not self.bot:
            ...
            if isinstance(bot, BotU):
                self.bot = bot
        
        assert self.bot is not None
        
        cog = self.bot.get_cog('R6 Commands')
        if not isinstance(cog, ApiCog):
            raise ValueError("Cog not found")
        
        user = await cog.fetch_r6_user(name=self.username, uid=self.id, platform=self.platform, fetch=True)
        _, _, _, ranked_stats, _ = await user.load_ranked_v2()

        embed = cog.generate_lookup_embed(self.gamemode, self.platform, user.name, None, ranked_stats)
        msg: Optional[discord.Message] = interaction.message
        if not msg:
            raise ValueError("Message not found")
        view = CustomBaseView(msg)
        for button in msg.components:
            if isinstance(button, discord.ui.Item):
...
class ApiCog(CogU, name='R6 Commands'):
    ...
    async def r6_lookup_cmd(self, ctx: ContextU, gamemode: str, platform: PlatformConverter, *, username: str, fetch: bool=False):
        ...
        #embed = self.generate_lookup_embed(gamemode, platform, player_name, cached_info, player_obj)
        ...
        if player_obj:
            ...
            rank_name = cached_info.ranked_rank
        else:
            raise ValueError("Failed to get rank")
        
        if gamemode == 'ranked':
            rankobj = R6Rank.from_str(rank_name)
            color = rankobj.color
        ...
        #         label="View on R6Tracker",
        #         url=get_perma_r6_tracker_url(id, platform),
        #     )
        # )
        #view.add_item(RefreshButton(gamemode, platform, id))
        ...
        if not ctx.interaction:
            ...
            await msg.reply(embed=emb2, delete_after=10.0)
    
    def generate_lookup_embed(self, gamemode: str, platform: Platform, player_name: str, cached_info: Optional[RankedStats]=None, player: Optional[FullProfile]=None):
        if player:
            wins = player.wins
