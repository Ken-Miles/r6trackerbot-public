...
from utils import BotU, ContextU, makeembed_failedaction
...
class HelpView(BaseView):
    ...
    def embed(self) -> discord.Embed:
        ...
        embed = discord.Embed(
            title="Bot Help Menu",
            description=f"Hello, I'm {self.context.bot.user.mention}! A bot to aid in the searching and display of statistics relating to Rainbow Six Siege.",  # type: ignore
        )
        embed.set_author(
...
class Help(commands.HelpCommand):
    ...
    async def send_cog_help(self, cog: Cog, /) -> discord.Message:  # type: ignore
        ...
        if getattr(cog, 'hidden', True) and not await self.context.bot.is_owner(self.context.author):
            emb = makeembed_failedaction(description="You do not have permission to view these commands.")
            view.remove_item()
